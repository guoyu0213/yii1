...
body{width:100%;}
<?php }else{?>
body{width:700px;overflow-y:hidden}
<?php }?>
</style>
...
<div class="pop_win">
	<form action='<?php echo IUrl::creatUrl("/goods/spec_update");?>' method='post' id='specForm' name='specForm'>
		...
		<table class="form_table">
			...
			<tr>
				<td>规格名称：</td>
				<td><input class="small" name="name" value="<?php echo isset($name)?$name:"";?>" type="text" pattern="required" alt="名字不能为空" /><label>* 规格名称（必填）</label></td>
			</tr>
			<tr>
				<td>显示类型：</td>
				<td>
			...
			<tr>
				<td>说明：</td>
				<td><input class="small" type="text" name="note" value="<?php echo isset($note)?$note:"";?>" /></td>
			</tr>
			<tr>
				<td></td>
				<td><button type="button" class="btn" onclick="addSpec();"><span class="add">添加规格</span></button></td>
			</tr>
			<tr>
				<td colspan="2">
...
<tr class='td_c'>
	<td>
		...
		<div style="display:<%=imgType%>" name="imageBox">
			<div class="imgbox"><img class="img_border" src='<%=webroot(show)%>' width='50px' height='50px' <%if(!show || type != 2){%>style="display:none;"<%}%> /></div>
...
function updatePic(indexValue,srcValue)
{
	...
	$('#spec_box tr:eq('+indexValue+')').find(".img_border").attr("src",imageUrl);
