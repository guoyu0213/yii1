					...
					<td>
						<table class="list_table2" width="100%">
							...
							<tbody>
								...
								<tr><th>销售总额</th><td><?php $query = new IQuery("order");$query->fields = "sum(order_amount) as amount";$query->where = "`pay_status` = 1";$items = $query->find(); foreach($items as $key => $item){?><a href="<?php echo IUrl::creatUrl("/market/amount");?>"><b class="f14 red3"><?php echo empty($item['amount']) ? 0 : $item['amount'];?></b> 元</a><?php }?></td></tr>
