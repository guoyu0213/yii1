...
class _userInfo extends pluginBase
{
	...
	public function saveCallback()
	{
		...
		$notCallback = array('/simple/reg','/simple/login','bind_user','oauth_callback');
		if($callback)
		{
			foreach($notCallback as $key => $url)
			{
				if(strpos($callback,$url) !== false)
				{
					$callback = '';
				}
			}
			$callback ? ISession::set('callback',rtrim($callback,"/")) : '';
		}
	}
	...
	public function userRegAct()
	{
		...
		// $inviteUser = IFilter::act(base64_decode(IFilter::act(IReq::get('inviteUser','post'))),'int');
		// var_dump($inviteUrl);die;
		// $userArray['inviteUser'] = $inviteUser;
